...
def _hf_token_for_pretrained() -> str | bool | None:
    """
    HF_TOKEN / HUGGING_FACE_HUB_TOKEN explícitos.
    Si no hay ninguno: None (sin auth explícita; el Hub puede usar caché).
    Evita pasar token=True, que en algunas versiones dispara login interactivo.
    """
...
def _from_pretrained_with_auth(cls: Any, model_name: str, **kwargs: Any) -> Any:
    """transformers antiguos usan use_auth_token en lugar de token."""
...
# Alias por compatibilidad con código que esperaba SiglipSO400MClassifier

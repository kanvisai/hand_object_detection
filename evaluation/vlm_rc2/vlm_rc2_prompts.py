"""
Prompts SigLIP para el pipeline rc2 (copiable a otro proyecto tal cual).
...
`_session_semantics` / `robbery_rules_score`.
...
def assert_prompts_ok() -> None:
    if len(RC2_PROMPT_TEXTS_EN) != EXPECTED_PROMPTS:
        raise RuntimeError(
            f"vlm_rc2_prompts: se esperan {EXPECTED_PROMPTS} prompts; hay {len(RC2_PROMPT_TEXTS_EN)}."
        )

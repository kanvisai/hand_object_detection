#!/usr/bin/env python3
"""
Agrega JSON de semántica rc2 y aplica `robbery_rules_score`.

100 % autosuficiente — todos los imports residen dentro de vlm_rc2/.

Entrada (exactamente una):
  - `--chunk-main-dir`: árbol de chunks con *_vlm.json por carpeta.
...
Modo producción: ante datos ausentes o errores graves, escribe `vlm_evaluation_error.json`
...
from _session_semantics import discover_chunk_dirs_under_parent  # noqa: E402
...
from vlm_rc2_errors import evaluation_unavailable_payload  # noqa: E402
from vlm_rc2_metrics import (  # noqa: E402
    mean_probability_pct,
    object_in_hand_probs_evaluable_frames,
)
from vlm_rc2_prompts import PROMPT_VARIANT_ID  # noqa: E402
...
def _collect_frames_from_ordered_semantics(
    ...
    for label, data in ordered:
        if str(data.get("vlm_rc2_pipeline_status") or "") == "error":
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="vlm_rc2: reglas de robo agregadas sobre JSON de semántica.")
    p.add_argument(
        "--chunk-main-dir",
        "--chunk_main_dir",
        default="",
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Repite el flag por cada chunk en orden temporal."
        ),
    )
    p.add_argument(
        "--semantics-json-file",
    ...
    *,
    ...
    if not args.no_write_file:
        try:
            ...
            _log(f"[vlm_rc2] No se pudo escribir {out_path}: {w}")
    ...
    try:
        ...
        if cm:
            ...
            if missing and args.strict:
                _log("[vlm_rc2] [strict] Faltan JSON de semántica:\n  " + "\n  ".join(missing))
            ...
            if not discovered_semantics_labels:
                ...
                _emit_degraded(args=args, root=root, payload=payload, want_error_file=True, _log=_log)
                _log("[vlm_rc2] Sin datos de semántica; salida degradada (robbery_probability=-1).")
                return
        ...
        payload = {
            "schema_version": "vlm_rc2_aggregate_rules_1.4",
            "vlm_rc2_pipeline_status": pipeline_status,
        ...
        _log(
            f"[vlm_rc2] Evaluación guardada: {out_path.resolve()}"
            if not args.no_write_file
            else "[vlm_rc2] Solo stdout (--no-write-file)."
        )
        _log(
            f"[vlm_rc2] robbery_probability={payload['robbery_probability']} "
            f"last_chunk_object_visible_hands_pct={payload['last_chunk_object_visible_hands_pct']} "
            f"status={pipeline_status}"
        )

    except Exception as e:
        _log(f"[vlm_rc2] ERROR interno (salida degradada, exit 0): {e}\n{traceback.format_exc()}")
        root_fallback = Path(str(args.chunk_main_dir or "").strip()).expanduser().resolve() if cm else Path.cwd()
        payload = evaluation_unavailable_payload(
            chunk_main_dir=root_fallback,
            pipeline_status="error",
            reason=f"excepción en aggregate_rules_rc2: {e}",

...
class DifferentialOpenClipLikeClassifier(OpenClipLikeClassifier):
    """OpenCLIP/MobileCLIP con batch multiescala y score diferencial."""
    ...
    def _score_crops_batch(self, crops: list[np.ndarray]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        pv = torch.stack([self.preprocess(Image.fromarray(cv2.cvtColor(c, cv2.COLOR_BGR2RGB))) for c in crops], dim=0).to(self.device)
        img_feat = self._l2_normalize(self.model.encode_image(pv))
    ...
    def predict_yes_prob(
        ...
        self.last_prompt_used = "OpenCLIP diferencial: batch multiescala torso+manos."
            ...
            vlm_calls.append(
                {
                    ...
                    "stage": "openclip_batch3_differential",
...
def main() -> None:
    ...
    clf = DifferentialOpenClipLikeClassifier(
        args.vlm_model,
        args.device,
        args.vlm_prompt,
